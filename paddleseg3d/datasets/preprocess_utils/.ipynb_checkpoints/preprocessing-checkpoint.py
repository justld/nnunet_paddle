...
class GenericPreprocessor:
    ...
    def run(self, target_spacings, input_folder_with_cropped_npz, output_folder, data_identifier,
            num_threads=1, force_separate_z=None):
        """
        ...
        list_of_cropped_npz_files = [os.path.join(input_folder_with_cropped_npz, file_name) for file_name in os.listdir(input_folder_with_cropped_npz) if os.path.isfile(os.path.join(input_folder_with_cropped_npz, file_name)) and file_name.endswith('.npz')]
        ...
        # all_classes = load_pickle(os.path.join(input_folder_with_cropped_npz, 'dataset_properties.pkl'))['all_classes']
        with open(os.path.join(input_folder_with_cropped_npz, 'dataset_properties.pkl'), 'rb') as f:
