...
from paddleseg3d.utils import (TimeAverager, calculate_eta, resume, logger,
                               worker_init_fn, train_profiler, op_flops_run, sum_tensor,
                               loss_computation)
...
def train(model,
    ...
    while iter < iters:
        for data in loader:
            ...
            images = data[0]
            if not isinstance(train_dataset, MSDDataset):
                labels = data[1].astype('int32')
            else:
                labels = data[1]

            if hasattr(model, 'data_format') and model.data_format == 'NDHWC' and not isinstance(train_dataset, MSDDataset):
            ...
            if precision == 'fp16':
                with paddle.amp.auto_cast():
                    ...
                    loss = sum(loss_list)
                    # print('loss:', loss.numpy(), logits_list[0][0].min().numpy(), logits_list[0][0].max().numpy())
                ...
                loss_list, per_channel_dice = loss_computation(
                    logits_list=logits_list, labels=labels, losses=losses)
                # print("train per_channel_dice: ", per_channel_dice)
